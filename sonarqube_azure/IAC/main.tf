...
resource "azurerm_postgresql_database" "sonardb" {
  name                = "sonarqubedb"
  resource_group_name = azurerm_resource_group.RG.name
...
data "template_file" "firstboot" {
  template = file("artifacts/firstboot.yaml")
}

data "template_file" "firstbootjump" {
  template = file("artifacts/firstbootjump.yaml")
}
